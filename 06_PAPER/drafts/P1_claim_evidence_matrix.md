...
|---|---|---|---|---|---|
| C1 | Same Bonneville run + same CCIW (*n*=1614): *R*² stays ~0.508–0.551 while NSE spans −2.804 / +0.500 / −2.752 across calibers A/B/C | Observational skill | Table 1; Figs 1–3; Fig. 4 (Bonneville points) | `w3_tdgta_off_metrics.json`; `cards/bonneville_tdgta_on.*` | Case-specific; *R*² and NSE describe different properties; does not alone prove provenance |
| C1b | DeGray T2 vs Tvolavg: *R*²=0.9027, NSE=−0.5855 (*n*=2943); STR vs GATE: *R*²≈0.53, NSE=−6.58 | Internal consistency | Table 4; Figs D1–D3; Fig. 3b; Fig. 4 | `w1_provenance_metrics.json`; `cards/degray_t_internal.*` | **Not** field skill; no T obs in deck |
| C1c | Columbia DO station pairs: highest-*R*² pair *R*²=0.6505, NSE=−1.4821 (*n*=116); wrong station > wrong layer (SNP sfc/bot NSE=0.91) | Internal consistency | Table 4; Figs C1–C2; Fig. 3c; Fig. 4 | `w1_provenance_metrics.json`; `cards/columbia_do_internal.*` | **Not** field skill; no DO obs in deck |
| C1d | Literature: **VPR-core** yes 2/38 (5.3%); `vpr_variable=yes` 0/38 (file/column); Table 2 W2↔obs skill 1/12 (Lima Neto 0.32); *R*² without NSE 9/11 | Literature gap | Table 2; Fig. 4 rug | `w5_lit_audit_summary.json`; `w5_lit_audit.csv` | Full text **9/38**; `unknown` not upgraded to absence; VPR-core ≠ full eight-field VPR |
...
Freeze status: **local pre-sub 2026-08-16**. Update only if JSON regenerates change a headline number.
